...
- [sessionscope](https://github.com/devilking7x/sessionscope) — Local-first explorer for Claude-style .jsonl session transcripts: drag & drop import with a tolerant parser, searchable transcript viewer, per-session token & cost stats with editable model pricing, full-text search across sessions, 2-session compare, and Markdown export. No backend. [Live demo](https://devilking7x.github.io/sessionscope/)
